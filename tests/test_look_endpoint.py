...
def fake_captions(monkeypatch):
    ...
    # main.py pulls the ML/scene stack (scenedetect, ultralytics...) at import;
    # the thin CI env does not have it, so these two tests skip there.
...
class TestStyledSubtitleEndpoint:
    def test_caption_styles_and_fonts_are_public(self, job):
        ...
        body = styles.json()
        assert len(body["presets"]) == 23 and body["default"] == "bold_white"   # 19 + Phase 5 + film_pop
